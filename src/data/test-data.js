...
const { loadDataFromCsv } = require('../utils/csvDataLoader');

const data = loadDataFromCsv();

// Environment variable overrides (kept from the original file, since these
// aren't really "test data" but environment config)
data.url.homePageUrl = process.env.BASE_URL || data.url.homePageUrl;
data.url.loginPageUrl = process.env.LOGINPAGE_URL || data.url.loginPageUrl;
data.url.productPageUrl = process.env.PRODUCTPAGE_URL || data.url.productPageUrl;
data.url.cartPageUrl = process.env.CARTPAGE_URL || data.url.cartPageUrl;
